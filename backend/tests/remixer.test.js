...
describe('POST /api/remixer', () => {
  ...
  beforeAll(async () => {
    ...
    fs.writeFileSync(path.join(fixturesDir, 'test.png'), Buffer.from('dummy png content'));
  ...
  describe('Success paths', () => {
    it('should process image and return mock captions when using mock path', async () => {
      ...
      const res = await request(app)
        .post('/api/remixer')
        .attach('image', path.join(fixturesDir, 'test.png'));
    ...
    it('should process image and return captions when using real Gemini API', async () => {
      if (!hasValidApiKey) {
        return;
      }
  ...
  describe('Error paths', () => {
    it('should return 400 if image field is missing', async () => {
      ...
      expect(res.body).toEqual({ error: 'Fichier image obligatoire.' });
    });

    it('should return 400 if invalid file type is uploaded', async () => {
      const res = await request(app)
        .post('/api/remixer')
        .attach('image', path.join(fixturesDir, 'invalid.txt'));

      expect(res.statusCode).toBe(400);
      expect(res.body.error).toContain('Seules les images');
    });
    ...
    it('should return 413 if file is larger than 8MB', async () => {
      ...
      expect(res.body.error).toContain('Le fichier image est trop volumineux');
    });
